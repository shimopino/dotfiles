...
- [mise](https://mise.jdx.dev/) 設定とタスクスクリプト
  - ツール定義は `conf.d/` に分割（runtimes / cloud / dev-tools / shell-tools）
  - `tasks/worktree/`: git worktree の作成・オープン・削除
  - `tasks/dotfiles/sync`（alias: `dotfiles-sync`）: chezmoi のドリフト確認 → re-add/apply → commit & push をインタラクティブに行う
- [sheldon](https://sheldon.cli.rs/) プラグイン設定
  - プロンプトは [Pure](https://github.com/sindresorhus/pure)（sheldon 経由で読み込み。旧 Starship から移行済み）
